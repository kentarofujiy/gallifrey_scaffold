  ...
  def create_htm_model
    ...
    rmodel1 = "@#{sorted_model[1]}.#{sorted_model[0].pluralize}"
     ...
     # filename = Dir.glob("db/migrate/*#{migration_name}.rb")[0]
      ...
      inject_into_file("app/views/#{sorted_model[0].pluralize}/show.html.erb", "\n  <ul><%=  @#{sorted_model[0]}.#{sorted_model[1].pluralize}.each do |ba| %><li><a href=""/#{sorted_model[1].pluralize}/<%= ba.id %>""><%= ba.attributes.keys %>:<%= ba.attributes.values %></li><% end %></ul>", :after => "<!-- Gallifrey_scaffold - AddField - Field - Do not remove -->")    
      inject_into_file("app/views/#{sorted_model[1].pluralize}/show.html.erb", "\n  <ul><%=  @#{sorted_model[1]}.#{sorted_model[0].pluralize}.each do |ba| %><li><a href=""/#{sorted_model[0].pluralize}/<%= ba.id %>""><%= ba.attributes.keys %>:<%= ba.attributes.values %></li><% end %></ul>", :after => "<!-- Gallifrey_scaffold - AddField - Field - Do not remove -->")   
