     ...
     # filename = Dir.glob("db/migrate/*#{migration_name}.rb")[0]
      ...
      inject_into_file("app/models/#{sorted_model[0]}.rb", "#{sorted_model[1]}_ids[], ",  :after => "def self.permitted_attributes\n    return ")
        ...
        inject_into_file("app/views/#{sorted_model[0].pluralize}/_form.html.erb", '
         <%= f.collection_check_boxes :#{sorted_model[1]}_ids, #{sorted_model[1].capitalize}.all, :id, :primary do |cb| %>
                <% cb.label(class: "checkbox-inline input_checkbox col-sm-2", style: "float: none") {cb.check_box(class: "checkbox") + cb.text} %>
            <% end %>',  :after => "<!-- Gallifrey Scaffold Menu Do Not Touch This -->" )
