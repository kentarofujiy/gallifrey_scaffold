# encoding : utf-8
class GallifreyMigrationGenerator < Rails::Generators::Base
  ...
  def generate_model
    generate("migration", "#{name} #{gallifrey_attr_to_rails_attr(true).join(' ')} #{@fulltext_field.join(' ')}")
  end
  ...
  def generate_views
    ...
    # Index
    inject_into_file("#{commonpath}index.html.erb", render_partial("app/views/partials/_index_batch.html.erb"), :before => "<!-- Gallifrey_scaffold - AddField - Batch - Do not remove -->\n" )
    inject_into_file("#{commonpath}index.html.erb", render_partial("app/views/partials/_index_header.html.erb"), :before => "<!-- Gallifrey_scaffold - AddField - Header - Do not remove -->\n" )
    inject_into_file("#{commonpath}index.html.erb", render_partial("app/views/partials/_index_column.html.erb"), :before => "<!-- Gallifrey_scaffold - AddField - Column - Do not remove -->\n" )
    inject_into_file("#{commonpath}index.html.erb", render_partial("app/views/partials/_index_search.html.erb"), :before => "<!-- Gallifrey_scaffold - AddField - Search - Do not remove -->\n" )
    inject_into_file("#{commonpath}index.html.erb", myattributes.map{ |attr| a,t = attr.split(':');"'#{a}'" }.join(',') + ',', :after => ":model_columns => [" )
    # Show
    inject_into_file("#{commonpath}show.html.erb", render_partial("app/views/partials/_show_field.html.erb"), :before => "<!-- Gallifrey_scaffold - AddField - Field - Do not remove -->\n" )    
  end
